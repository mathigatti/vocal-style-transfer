...
def sample_train_data(dataset_A, dataset_B, n_frames = 512):
...
def sample_train_data_for_spec(dataset_A, dataset_B, n_frames = 64):
...
def wavs_to_specs_train(wavs, n_fft = 1023, hop_length = 512):
    offset = 1e-6
    stfts = list()
    for wav in wavs:
        spec = stft(wav, n_fft=n_fft, hop_length=hop_length, win_length=n_fft)
        mag = np.abs(spec)
        
        mag = np.log(mag + offset)
        
        stfts.append(mag)

    return stfts

def wavs_to_specs_test(wavs, n_fft = 1023, hop_length = 512):
    offset = 1e-6
    stfts = list()
    phases = list()
    for wav in wavs:
